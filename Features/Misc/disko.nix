...
let
  inherit (lib) mkOption mkIf mkEnableOption mkMerge optionalAttrs optional genAttrs;
  inherit (lib.types) bool str coercedTo submodule;
  ...
  neededForBoot = [ "/nix" "/persistent" ] ++ optional fastEnabled "/fast";
in
{
  options.features.disko = {
    enable = mkEnableOption "declarative disk layout (disko)";
    isSSD = {
      primary = mkOption { type = bool; default = true; description = "Whether the primary drive is an SSD (enables discard=async and ssd mount options)"; };
      secondary = mkOption { type = bool; default = true; description = "Whether the secondary drive is an SSD (enables discard=async and ssd mount options)"; };
    };
    dualDrive = mkOption {
      description = "Spread /nix and /persistent across separate drives";
      default = { enable = false; fastDirs = false; };
      type = coercedTo bool
        (enable: { inherit enable; fastDirs = false; })
        (submodule {
          options = {
            enable = mkOption { type = bool; default = false; description = "Spread /nix and /persistent across separate drives"; };
            fastDirs = mkOption { type = bool; default = false; description = "Give /fast its own subvolume on the primary drive"; };
          };
        });
    };
    primaryDrive = mkOption { type = str; default = ""; description = "Primary/fastest drive"; example = "/dev/nvme0n1"; };
    secondaryDrive = mkOption { type = str; default = ""; description = "Secondary drive for /persistent (dual-drive mode)"; example = "/dev/nvme1n1"; };
    compression = mkOption { type = str; default = "zstd:3"; description = "Btrfs compression algorithm and level"; };
    swap = {
      enable = mkOption { type = bool; default = false; description = "Enable an optional swap partition"; };
      size = mkOption { type = str; default = "8G"; description = "Swap partition size"; };
    };
  };
  ...
  config = mkIf diskoCfg.enable {
    ...
    fileSystems = genAttrs neededForBoot (m: { neededForBoot = true; });
  };
}